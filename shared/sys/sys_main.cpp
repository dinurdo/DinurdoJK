									...
									size_t numPaths, uint32_t searchFlags, const char *callerName )
{
	...
	if (searchFlags & SEARCH_PATH_DINURDOJK)
	{
		for (size_t i = 0; i < numPaths; i++)
		{
			const char *libDir = searchPaths[i];
			...
			fn = FS_BuildOSPath(libDir, DINURDOJKGAME, filename);
			libHandle = Sys_LoadLibrary(fn);
			if (libHandle)
				return libHandle;
...
#endif
	{
		...
		if ( !libHandle )
		{
			...
			if (!libHandle) {
				...
				libHandle = Sys_LoadDllFromPaths( filename, gamedir, searchPaths, numPaths, SEARCH_PATH_BASE | SEARCH_PATH_DINURDOJK | SEARCH_PATH_MOD, __FUNCTION__ );
				if ( !libHandle )
					return NULL;
			}
		}
